...
static int Column_ChangeState(TreeCtrl *tree, Item *item, Column *column, int columnIndex, int stateOff, int stateOn)
{
	int cstate, state;
	int sMask, iMask = 0;
	...
	Tcl_Obj *CONST objv[])
{
	...
	switch (index)
	{
		...
		case COMMAND_FORCOLUMN:
		{
			...
			if (TreeItem_FromObj(tree, objv[4], (TreeItem *) &item, 0)
					!= TCL_OK)
				return TCL_ERROR;
